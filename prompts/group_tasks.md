...
# TASK
1. Read the specific phase document `docs/plan/phases/{phase_filename}`.
...
5. You MUST verify that 100% of the requirements for this phase were correctly grouped by running `python .tools/verify.py json ../phases/{phase_filename} ../tasks/{group_filename}`.
...
1. Use your tools to read `docs/plan/phases/{phase_filename}` and extract the exact set of active requirement IDs.
2. Categorize the extracted requirements logically based on functional dependencies.
3. Write the resulting JSON object to `docs/plan/tasks/{group_filename}`.
...
# OUTPUT FORMAT
- Your output MUST ONLY be a valid JSON file saved to `docs/plan/tasks/{group_filename}`.
