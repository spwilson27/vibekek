...
def test_claude_runner_get_cmd():
    runner = ClaudeRunner()
    assert runner.get_cmd() == ["claude", "-p", "--dangerously-skip-permissions", "--output-format", "stream-json"]
    assert runner.get_cmd(image_paths=["/a.png", "/b.jpg"]) == [
        "claude", "-p", "--dangerously-skip-permissions", "--output-format", "stream-json", "--image", "/a.png", "--image", "/b.jpg"
    ]
