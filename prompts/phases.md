...
- **Phase 0 must be the first phase.** It establishes the project scaffolding so `./do presubmit` passes. See the project roadmap for Phase 0 requirements. Phase 0 is not gated by presubmit, but all subsequent phases are — the Definition of Done for Phase 0 is that `./do presubmit` passes.
