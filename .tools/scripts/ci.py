...
class CiOrchestrator:
    ...
    def _push_temp_and_trigger(self, branch: str, original_dir: str):
        ...
        if not push_remote:
            try:
                ...
                pass
        if not push_remote:
            push_remote = f"https://{self.host}/{self.project_path}.git"
        subprocess.run(["git", "push", push_remote, f"HEAD:{temp_branch}"], cwd=original_dir)
        pushed = False
        for _ in range(5):
            try:
        ...
        return self.provider.trigger_pipeline(self.host, self.project_encoded, temp_branch, self.token)
    ...
    def _handle_clean(self):
        ...
        found = self.provider.search_pipeline(self.host, self.project_encoded, sha, branch, None, self.token)
        if found:
        ...
        try:
            self.pipeline_id, self.web_url, self.created_at = self.provider.trigger_pipeline(self.host, self.project_encoded, target_branch, self.token)
    ...
    def _handle_dirty(self):
        ...
        existing = self.provider.search_pipeline(self.host, self.project_encoded, base_sha, None, diff_md5, self.token)
        if existing:
        ...
        if found:
            ...
            return
        try:
            self.pipeline_id, self.web_url, self.created_at = self.provider.trigger_pipeline(self.host, self.project_encoded, branch, self.token)
        except RuntimeError as e:
            msg = str(e)
            if "Reference not found" in msg or "missing 'id'" in msg:
                self.pipeline_id, self.web_url, self.created_at = self._push_temp_and_trigger(branch, self.original_dir)
    ...
    def _monitor(self):
        ...
        while True:
            self.provider.poll_logs(self.host, self.project_encoded, self.pipeline_id, self.token, cursors)
            s = self.provider.check_pipeline_status(self.host, self.project_encoded, self.pipeline_id, self.token)
            if s == "success":
                return
            if s in ("failed", "canceled", "skipped"):
                if s == "failed":
                    self.provider.handle_failure(self.host, self.project_encoded, self.pipeline_id, self.token, self.original_dir)
    ...
    def run(self):
        ...
        while True:
            ...
            if now - last_poll >= 5:
                try:
                    self.provider.poll_logs(host, project_encoded, self.pipeline_id, token, cursors)
                except Exception as e:
                    print(f"  ! Failed to poll logs: {e}")

                try:
                    status = self.provider.check_pipeline_status(host, project_encoded, self.pipeline_id, token)
                except Exception:
                    status = "unknown"
                ...
                if status in ("failed", "canceled", "skipped"):
                    ...
                    if status == "failed":
                        try:
                            self.provider.handle_failure(host, project_encoded, self.pipeline_id, token, self.original_dir)
                        except Exception:
                            pass
                    raise RuntimeError("Pipeline failed")
...
class RealCiProvider:
    ...
    def is_canceled(self) -> bool:
        # Real provider doesn't have a cancellation mechanism in this script,
        # so default to False. The run loop will check this method if present.
        return False
