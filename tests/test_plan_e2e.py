...
TASK_SIDECAR_A = {
    "task_id": "phase_1/01_setup_auth.md",
    "phase": "phase_1",
    "type": "task",
    "depends_on": [],
...
TASK_SIDECAR_B = {
    "task_id": "phase_1/02_user_profile.md",
    "phase": "phase_1",
    "type": "task",
    "depends_on": ["phase_1/01_setup_auth.md"],
    "feature_gates": ["features/auth_basic_login"],
    "requirement_mappings": ["1_PRD-REQ-002"],
    "epic_id": "auth"
}

DAG = {
    "phase_1/01_setup_auth.md": [],
    "phase_1/02_user_profile.md": ["phase_1/01_setup_auth.md"],
}
...
def _pre_populate_plan_artifacts(tmp_path):
    ...
    phase_dir = tasks_dir / "phase_1"
    phase_dir.mkdir(parents=True, exist_ok=True)

    # Spec and research documents + summaries (specs only get summaries)
    ...
    for doc in DOCS:
        ...
        doc_dir = plan_dir / out_folder
        doc_dir.mkdir(parents=True, exist_ok=True)
        (doc_dir / f"{doc['id']}.md").write_text(f"# {doc['name']}\nStub.\n")
        if doc["type"] == "spec":
    ...
    # Per-doc requirement extractions (specs only)
    req_dir = plan_dir / "requirements"
    req_dir.mkdir(parents=True, exist_ok=True)
    spec_docs = [d for d in DOCS if d["type"] == "spec"]
    ...
    # Task files with sidecars (holistic tasks, flat in phase dir)
    (phase_dir / "01_setup_auth.md").write_text("# Task: Setup Auth\nImplement auth system.\n")
    (phase_dir / "01_setup_auth.json").write_text(json.dumps(TASK_SIDECAR_A, indent=2))
    (phase_dir / "02_user_profile.md").write_text("# Task: User Profile\nProfile management.\n")
    (phase_dir / "02_user_profile.json").write_text(json.dumps(TASK_SIDECAR_B, indent=2))
    ...
    # DAG files (keys are relative to phase dir)
    phase_1_dag = {
        "01_setup_auth.md": [],
        "02_user_profile.md": ["01_setup_auth.md"],
    }
    (tasks_dir / "phase_1" / "dag.json").write_text(json.dumps(phase_1_dag, indent=2))
...
class TestPlanningE2EHappyPath:
    ...
    def test_orchestrator_runs_all_phases(self, tmp_path):
        ...
        agent = _make_agent({
            ...
            # Phase 15: Holistic tasks produce sidecars
            "holistic": json.dumps(TASK_SIDECAR_A),
        })
...
class TestValidation:
    ...
    def test_ordered_requirements_missing_ids(self, tmp_path):
        ...
        rc, out = self._run_validate(tmp_path, phase=11)
    ...
    def test_task_sidecar_invalid_type(self, tmp_path):
        """Invalid task type (not 'task') is caught."""
        plan_dir = tmp_path / "docs" / "plan"
        task_dir = plan_dir / "tasks" / "phase_1"
        task_dir.mkdir(parents=True)

        bad_sidecar = dict(TASK_SIDECAR_A, type="blue")  # invalid type
        ...
        rc, out = self._run_validate(tmp_path, phase=15)
    ...
    def test_dag_cycle_detection(self, tmp_path):
        ...
        rc, out = self._run_validate(tmp_path, phase=19)
    ...
    def test_cross_phase_duplicate_task_ids(self, tmp_path):
        ...
        for phase in ["phase_1", "phase_2"]:
            ...
            sidecar = dict(TASK_SIDECAR_A, task_id="DUPLICATE_ID", phase=phase)
        ...
        rc, out = self._run_validate(tmp_path, phase=17)
        assert rc != 0, f"Expected validation failure for duplicate task_id"
        assert "Duplicate" in out or "duplicate" in out

    def test_task_sidecar_valid_feature_gates(self, tmp_path):
        """Task with valid feature gates passes validation."""
        plan_dir = tmp_path / "docs" / "plan"
        task_dir = plan_dir / "tasks" / "phase_1"
        task_dir.mkdir(parents=True)

        # Cover both requirements across two tasks
        sidecar_a = dict(TASK_SIDECAR_A, feature_gates=["features/auth_login"])
        (task_dir / "01_task.json").write_text(json.dumps(sidecar_a))
        (task_dir / "01_task.md").write_text("# Task A\n")

        sidecar_b = dict(TASK_SIDECAR_B, feature_gates=["features/auth_login"])
        (task_dir / "02_task.json").write_text(json.dumps(sidecar_b))
        (task_dir / "02_task.md").write_text("# Task B\n")

        # Also need merged requirements for traceability check
        (plan_dir / "requirements.json").write_text(json.dumps(MERGED_REQS, indent=2))
        ...
        rc, out = self._run_validate(tmp_path, phase=15)
        assert rc == 0, f"Expected validation to pass: {out}"
    ...
    def test_traceability_all_requirements_tested(self, tmp_path):
        ...
        sidecar = dict(TASK_SIDECAR_A, requirement_mappings=["1_PRD-REQ-001", "1_PRD-REQ-002"])
        (task_dir / "01_task.json").write_text(json.dumps(sidecar))
        (task_dir / "01_task.md").write_text("# Task\n")

        # Green task produces the gate
        green = dict(TASK_SIDECAR_B, requirement_mappings=[])
    ...
    def test_traceability_untested_requirement(self, tmp_path):
        ...
        sidecar = dict(TASK_SIDECAR_A, requirement_mappings=["1_PRD-REQ-001"])
        (task_dir / "01_task.json").write_text(json.dumps(sidecar))
        (task_dir / "01_task.md").write_text("# Task\n")
    ...
    def test_traceability_contributes_to_not_counted(self, tmp_path):
        ...
        sidecar = dict(TASK_SIDECAR_A,
                       requirement_mappings=["1_PRD-REQ-001"],
                       contributes_to=["1_PRD-REQ-002"])
        (task_dir / "01_task.json").write_text(json.dumps(sidecar))
        (task_dir / "01_task.md").write_text("# Task\n")

        green = dict(TASK_SIDECAR_B,
                     requirement_mappings=[],
                     contributes_to=[])
        (task_dir / "02_green.json").write_text(json.dumps(green))
        (task_dir / "02_green.md").write_text("# Green Task\n")
    ...
    def test_task_sidecar_with_contributes_to_valid(self, tmp_path):
        ...
        sidecar = dict(TASK_SIDECAR_A, contributes_to=["1_PRD-REQ-002"])
        (task_dir / "01_task.json").write_text(json.dumps(sidecar))
        (task_dir / "01_task.md").write_text("# Task\n")
    ...
    def test_task_sidecar_without_contributes_to_valid(self, tmp_path):
        ...
        sidecar = dict(TASK_SIDECAR_A)  # No contributes_to
    ...
    def test_task_sidecar_contributes_to_invalid_pattern(self, tmp_path):
        ...
        sidecar = dict(TASK_SIDECAR_A, contributes_to=["not-a-valid-id"])
        (task_dir / "01_task.json").write_text(json.dumps(sidecar))
        (task_dir / "01_task.md").write_text("# Task\n")
    ...
    def test_task_requirement_mappings_within_cap(self, tmp_path):
        ...
        sidecar = dict(TASK_SIDECAR_A, requirement_mappings=[
...
# ---------------------------------------------------------------------------
# Status and Run integration tests
# ---------------------------------------------------------------------------


class TestStatusAfterPlan:
    """Verify the status subcommand works after plan completion."""

    def test_status_shows_all_tasks(self, tmp_path, capsys):
        """cmd_status prints all tasks grouped by phase with correct icons."""
        tasks_dir = tmp_path / "docs" / "plan" / "tasks"

        # Phase 0: pre-init
        p0 = tasks_dir / "phase_0"
        p0.mkdir(parents=True)
        pre_init_sidecar = {
            "task_id": "phase_0/00_pre_init",
            "phase": "phase_0",
            "type": "task",
            "depends_on": [],
            "feature_gates": [],
        ...
        (p0 / "00_pre_init.json").write_text(json.dumps(pre_init_sidecar))
        (p0 / "00_pre_init.md").write_text("# Pre-Init\n")
        (p0 / "dag.json").write_text(json.dumps({"00_pre_init.md": []}))

        # Phase 1: two tasks with dependency
        p1 = tasks_dir / "phase_1"
        p1.mkdir(parents=True)
        (p1 / "01_setup_auth.json").write_text(json.dumps(TASK_SIDECAR_A))
        (p1 / "01_setup_auth.md").write_text("# Setup Auth\n")
        (p1 / "02_user_profile.json").write_text(json.dumps(TASK_SIDECAR_B))
        (p1 / "02_user_profile.md").write_text("# User Profile\n")
        phase_1_dag = {"01_setup_auth.md": [], "02_user_profile.md": ["01_setup_auth.md"]}
        (p1 / "dag.json").write_text(json.dumps(phase_1_dag))

        wf_state = {"completed_tasks": [], "merged_tasks": []}
        rp_state = {"blocked_tasks": {}}
        wf_state_file = tmp_path / ".workflow_state" / "workflow_state.json"
        wf_state_file.parent.mkdir(parents=True)
        wf_state_file.write_text(json.dumps(wf_state))
        rp_state_file = tmp_path / ".workflow_state" / "replan_state.json"
        rp_state_file.write_text(json.dumps(rp_state))

        from workflow_lib.state import load_dags, load_workflow_state
        with patch("workflow_lib.replan.get_tasks_dir", return_value=str(tasks_dir)), \
             patch("workflow_lib.replan.load_dags", side_effect=lambda td: load_dags(str(tasks_dir))), \
        ...
        assert "phase_0" in captured.out
        assert "phase_1" in captured.out
        # All tasks should be pending ([ ] or [.])
        assert "0/3 completed" in captured.out or "0/" in captured.out

    def test_status_shows_completed_tasks(self, tmp_path, capsys):
        """Completed tasks show [x] icon when merged."""
        tasks_dir = tmp_path / "docs" / "plan" / "tasks"
        p1 = tasks_dir / "phase_1"
        p1.mkdir(parents=True)
        (p1 / "01_setup_auth.json").write_text(json.dumps(TASK_SIDECAR_A))
        (p1 / "01_setup_auth.md").write_text("# Setup\n")
        phase_1_dag = {"01_setup_auth.md": []}
        (p1 / "dag.json").write_text(json.dumps(phase_1_dag))

        wf_state = {
            "completed_tasks": ["phase_1/01_setup_auth.md"],
            "merged_tasks": ["phase_1/01_setup_auth.md"],
        }
        rp_state = {"blocked_tasks": {}}

        with patch("workflow_lib.replan.get_tasks_dir", return_value=str(tasks_dir)), \
             patch("workflow_lib.replan.load_dags") as mock_load_dags, \
             patch("workflow_lib.replan.load_workflow_state", return_value=wf_state), \
             patch("workflow_lib.replan.load_replan_state", return_value=rp_state):
            mock_load_dags.return_value = {"phase_1/01_setup_auth.md": []}
            from workflow_lib.replan import cmd_status
            cmd_status(MagicMock())
    ...
    def test_status_shows_blocked_tasks(self, tmp_path, capsys):
        """Blocked tasks show [B] icon with reason."""
        tasks_dir = tmp_path / "docs" / "plan" / "tasks"
        p1 = tasks_dir / "phase_1"
        p1.mkdir(parents=True)
        (p1 / "01_task.json").write_text(json.dumps(TASK_SIDECAR_A))
        (p1 / "01_task.md").write_text("# Task\n")
        (p1 / "dag.json").write_text(json.dumps({"01_task.md": []}))

        wf_state = {"completed_tasks": [], "merged_tasks": []}
        rp_state = {
            "blocked_tasks": {
                "phase_1/01_setup_auth.md": {"reason": "dependency missing"}
            }
        }
        ...
        assert "dependency missing" in captured.out


class TestRunEnumeratesDAG:
    """Verify DAG loading and task enumeration for the run command."""

    def test_load_dags_from_sidecar_files(self, tmp_path):
        """load_dags reads task_id and depends_on from JSON sidecars."""
        tasks_dir = tmp_path / "docs" / "plan" / "tasks"

        # Phase 0
        p0 = tasks_dir / "phase_0"
        p0.mkdir(parents=True)
        (p0 / "00_pre_init.json").write_text(json.dumps({
            "task_id": "phase_0/00_pre_init",
            "phase": "phase_0",
            "type": "task",
            "depends_on": [],
        ...
        # Phase 1 with dependencies
        p1 = tasks_dir / "phase_1"
        p1.mkdir(parents=True)
        (p1 / "01_setup_auth.json").write_text(json.dumps(TASK_SIDECAR_A))
        (p1 / "02_user_profile.json").write_text(json.dumps(TASK_SIDECAR_B))

        from workflow_lib.state import load_dags
        dag = load_dags(str(tasks_dir))

        assert "phase_0/00_pre_init" in dag
        assert dag["phase_0/00_pre_init"] == []
        assert "phase_1/01_setup_auth.md" in dag
        assert dag["phase_1/01_setup_auth.md"] == []
        assert "phase_1/02_user_profile.md" in dag
        assert dag["phase_1/02_user_profile.md"] == ["phase_1/01_setup_auth.md"]

    def test_load_dags_skips_dag_json(self, tmp_path):
        """dag.json files in phase dirs are not treated as task sidecars."""
        tasks_dir = tmp_path / "docs" / "plan" / "tasks"
        p1 = tasks_dir / "phase_1"
        p1.mkdir(parents=True)
        ...
        # Only the task sidecar should be in the DAG, not dag.json itself
        assert len(dag) == 1
        assert "phase_1/01_setup_auth.md" in dag

    def test_execute_dag_processes_all_tasks(self, tmp_path):
        """execute_dag calls process_task for each task in dependency order."""
        tasks_dir = tmp_path / "docs" / "plan" / "tasks"
        p1 = tasks_dir / "phase_1"
        p1.mkdir(parents=True)
        (p1 / "01_setup_auth.json").write_text(json.dumps(TASK_SIDECAR_A))
        ...
        from workflow_lib.state import load_dags
        master_dag = load_dags(str(tasks_dir))
        state = {"completed_tasks": [], "merged_tasks": []}

        completion_order = []

        def _fake_process(root_dir, task_id, presubmit_cmd, backend="gemini", **kwargs):
            completion_order.append(task_id)
            return True

        def _fake_merge(root_dir, task_id, presubmit_cmd, backend="gemini", **kwargs):
            return True

        # Initialize git repo for execute_dag
        ...
        subprocess.run(["git", "commit", "--allow-empty", "-m", "init"], cwd=str(tmp_path),
                       capture_output=True, env={**os.environ, "GIT_AUTHOR_NAME": "test",
                       "GIT_AUTHOR_EMAIL": "t@t", "GIT_COMMITTER_NAME": "test",
                       "GIT_COMMITTER_EMAIL": "t@t"})
        ...
        with patch("workflow_lib.executor.process_task", side_effect=_fake_process), \
             patch("workflow_lib.executor.merge_task", side_effect=_fake_merge), \
             ...
             patch("subprocess.run", return_value=MagicMock(returncode=0, stdout="dev", stderr="")):
            execute_dag(
                str(tmp_path), master_dag, state, jobs=1,
                presubmit_cmd="echo ok", backend="gemini",
            )

        # Both tasks should have been processed
        assert set(completion_order) == {"phase_1/01_setup_auth.md", "phase_1/02_user_profile.md"}
        # A must come before B (B depends on A)
        idx_a = completion_order.index("phase_1/01_setup_auth.md")
        idx_b = completion_order.index("phase_1/02_user_profile.md")
        assert idx_a < idx_b, f"Expected setup_auth before user_profile: {completion_order}"

    def test_dag_with_multiple_phases(self, tmp_path):
        """load_dags merges tasks from multiple phase directories."""
        tasks_dir = tmp_path / "docs" / "plan" / "tasks"

        for phase_num in range(3):
            phase_dir = tasks_dir / f"phase_{phase_num}"
            phase_dir.mkdir(parents=True)
            sidecar = {
                "task_id": f"phase_{phase_num}/task_01",
            ...
            (phase_dir / "task_01.json").write_text(json.dumps(sidecar))
        ...
        assert len(dag) == 3
        for phase_num in range(3):
            assert f"phase_{phase_num}/task_01" in dag
